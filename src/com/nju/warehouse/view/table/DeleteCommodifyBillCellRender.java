...
import com.nju.warehouse.controller.salesman.DeleteCommodityBillController;
...
public class DeleteCommodifyBillCellRender  extends AbstractCellEditor 
	...
	public DeleteCommodifyBillCellRender(JTable table, int column)
	{
	    super();
	    this.table = table;
	...
	public void actionPerformed(ActionEvent e)
	{
	    ...
	    if(type == CommodityBillType.DEL) {
	    	JOptionPane.showMessageDialog(table, "这是退货单。。乃还想退给谁！","退货提示",JOptionPane.ERROR_MESSAGE);
	    } else if(Boolean.parseBoolean(table.getValueAt(table.getSelectedRow(), 8).toString())) {
	    	JOptionPane.showMessageDialog(table, "已经退过了。。不要再退了！","退货提示",JOptionPane.ERROR_MESSAGE);
		}else{
	    	...
	    	CommodityBill bill = new CommodityBill(date, CommodityBillType.DEL, customer, name, CommodityType, amount, price, total,false);
	    	
	    	new DeleteCommodityBillController(SalesManView.getPurchasesManageView(), new DeleteCommodityBill(bill));
