...
import com.nju.warehouse.controller.salesman.AddCommodityBillController;
import com.nju.warehouse.result.AddResult;
import com.nju.warehouse.util.LogUtil;

@SuppressWarnings("serial")
public class AddCommodityBillView extends JFrame implements Observer{
	...
	public AddCommodityBillView() {
		LogUtil.getInstance().log(Level.INFO, "AddCommodityBillView---constructor");
		initView();
	}
	
	public void initView() {
		this.setTitle("进货");
		this.setSize(280, 490);
	...
	public void update(Observable o, Object arg) {
		if(arg == null) {
			LogUtil.getInstance().log(Level.INFO, "AddCommodityBillView--update--cancel");
			
			this.dispose();
		} else {
			LogUtil.getInstance().log(Level.INFO, "AddCommodityBillView--update--OK");
			
			AddResult result = (AddResult)arg;
			
			if(result == AddResult.添加成功) {
				this.dispose();
				JOptionPane.showMessageDialog(this, result.toString(), "进货提示", JOptionPane.CLOSED_OPTION);
			} else {
				JOptionPane.showMessageDialog(this, result.toString(), "进货提示", JOptionPane.ERROR_MESSAGE);
			}
		}
	...
	public void addListener(AddCommodityBillController controller) {
